...
def addtransaction(r):
    mydb = connect()
    mycursor = mydb.cursor()
    tx = 'select tid from tran order by tid desc limit 1'
    print(tx)
    mycursor.execute(tx)
    e = mycursor.fetchall()
    if len(e) == 0:
            ...
            eid = e[0][0]+1
    d="insert into tran(tid,prod_id,fromid,toid,transaddress,remark,file,filehash)values ('%s','%s',(select uid from users where blockchainaddress='%s'),(select uid from users where blockchainaddress='%s'),'%s','%s','%s','%s')"%(eid,r['prod_id'],r['fromid'],r['toid'],r['transaddress'],r['remark'],r["file"],r["filehash"])
    print(d)
    mycursor = mydb.cursor()
    mycursor.execute(d)
    mydb.commit()
...
def viewseedhistory():
        ...
        mydb = connect()
        mycursor = mydb.cursor()
        tx="select *   from tran  where toid=(select uid from users where blockchainaddress='%s') or fromid=(select uid from users where blockchainaddress='%s')"%(r["address"],r["address"])
        mycursor.execute(tx)
        e=mycursor.fetchall()
        mydb.close()
        return json.dumps(e)
...
def viewhistory():
        ...
        mydb = connect()
        mycursor = mydb.cursor()
        tx="select *   from tran  where prod_id='%s'"%(r["productid"])
        mycursor.execute(tx)
        e=mycursor.fetchall()
        mydb.close()
        return json.dumps(e)
...
def inserttransaction():
    ...
    if len(e) == 0:
            ...
            eid = e[0][0]+1
    d="insert into tran(tid,prod_id,fromid,toid,transactiondate,transaddress,remark)values ('%s','%s','%s','%s','%s','%s','%s')"%(eid,r['prod_id'],r['fromid'],r['toid'],r['transactiondate'],r['transaddress'],r['remark'])
    mycursor = mydb.cursor()
    mycursor.execute(d)
    mydb.commit()
    mydb.close()
...
def updatetransaction():
    r=request.json
    mydb = connect()
    d="update tran set prod_id ='%s',fromid ='%s',toid ='%s',transactiondate ='%s',transaddress ='%s',remark ='%s' where tid='%s'"%(r['prod_id'],r['fromid'],r['toid'],r['transactiondate'],r['transaddress'],r['remark'],r['tid'])
    mycursor = mydb.cursor()
    mycursor.execute(d)
    mydb.commit()
    mydb.close()
...
def deletetransaction():
        ...
        mydb = connect()
        mycursor = mydb.cursor()
        tx="delete from tran where tid={0}".format(r['id'])
        mycursor.execute(tx)
        mydb.commit()
        mydb.close()
        return 's'
